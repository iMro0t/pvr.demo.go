...
extern "C"
{
  ...
  typedef struct KodiToAddonFuncTable_PVR
  {
    ...
    PVR_ERROR(__cdecl *GetAddonCapabilities)
    (PVR_ADDON_CAPABILITIES *);
    PVR_ERROR(__cdecl *GetStreamProperties)
    ...
    PVR_ERROR(__cdecl *GetDriveSpace)
    (long long *, long long *);
    PVR_ERROR(__cdecl *MenuHook)
    (const PVR_MENUHOOK *, const PVR_MENUHOOK_DATA *);
    PVR_ERROR(__cdecl *GetEPGForChannel)
    (ADDON_HANDLE, const PVR_CHANNEL *, time_t, time_t);
    PVR_ERROR(__cdecl *IsEPGTagRecordable)
    (const EPG_TAG *, bool *);
    PVR_ERROR(__cdecl *IsEPGTagPlayable)
    (const EPG_TAG *, bool *);
    PVR_ERROR(__cdecl *GetEPGTagEdl)
    (const EPG_TAG *, PVR_EDL_ENTRY[], int *);
    PVR_ERROR(__cdecl *GetEPGTagStreamProperties)
    (const EPG_TAG *, PVR_NAMED_VALUE *, unsigned int *);
    int(__cdecl *GetChannelGroupsAmount)(void);
    PVR_ERROR(__cdecl *GetChannelGroups)
    (ADDON_HANDLE, bool);
    PVR_ERROR(__cdecl *GetChannelGroupMembers)
    (ADDON_HANDLE, const PVR_CHANNEL_GROUP *);
    PVR_ERROR(__cdecl *OpenDialogChannelScan)
    (void);
    int(__cdecl *GetChannelsAmount)(void);
    PVR_ERROR(__cdecl *GetChannels)
    (ADDON_HANDLE, bool);
    PVR_ERROR(__cdecl *DeleteChannel)
    (const PVR_CHANNEL *);
    PVR_ERROR(__cdecl *RenameChannel)
    (const PVR_CHANNEL *);
    PVR_ERROR(__cdecl *MoveChannel)
    (const PVR_CHANNEL *);
    PVR_ERROR(__cdecl *OpenDialogChannelSettings)
    (const PVR_CHANNEL *);
    PVR_ERROR(__cdecl *OpenDialogChannelAdd)
    (const PVR_CHANNEL *);
    int(__cdecl *GetRecordingsAmount)(bool);
    PVR_ERROR(__cdecl *GetRecordings)
    (ADDON_HANDLE, bool);
    PVR_ERROR(__cdecl *DeleteRecording)
    (const PVR_RECORDING *);
    PVR_ERROR(__cdecl *UndeleteRecording)
    (const PVR_RECORDING *);
    PVR_ERROR(__cdecl *DeleteAllRecordingsFromTrash)
    (void);
    PVR_ERROR(__cdecl *RenameRecording)
    (const PVR_RECORDING *);
    PVR_ERROR(__cdecl *SetRecordingLifetime)
    (const PVR_RECORDING *);
    PVR_ERROR(__cdecl *SetRecordingPlayCount)
    (const PVR_RECORDING *, int);
    PVR_ERROR(__cdecl *SetRecordingLastPlayedPosition)
    (const PVR_RECORDING *, int);
    int(__cdecl *GetRecordingLastPlayedPosition)(const PVR_RECORDING *);
    PVR_ERROR(__cdecl *GetRecordingEdl)
    (const PVR_RECORDING *, PVR_EDL_ENTRY[], int *);
    PVR_ERROR(__cdecl *GetTimerTypes)
    (PVR_TIMER_TYPE[], int *);
    int(__cdecl *GetTimersAmount)(void);
    PVR_ERROR(__cdecl *GetTimers)
    (ADDON_HANDLE);
    PVR_ERROR(__cdecl *AddTimer)
    (const PVR_TIMER *);
    PVR_ERROR(__cdecl *DeleteTimer)
    (const PVR_TIMER *, bool);
    PVR_ERROR(__cdecl *UpdateTimer)
    (const PVR_TIMER *);
    ...
    PVR_ERROR(__cdecl *SignalStatus)
    (PVR_SIGNAL_STATUS *);
    PVR_ERROR(__cdecl *GetDescrambleInfo)
    (PVR_DESCRAMBLE_INFO *);
    PVR_ERROR(__cdecl *GetChannelStreamProperties)
    (const PVR_CHANNEL *, PVR_NAMED_VALUE *, unsigned int *);
    ...
    PVR_ERROR(__cdecl *SetEPGTimeFrame)
    (int);
    ...
    PVR_ERROR(__cdecl *GetStreamTimes)
    (PVR_STREAM_TIMES *);
    PVR_ERROR(__cdecl *GetStreamReadChunkSize)
    (int *);
